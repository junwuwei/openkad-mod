...
public class KeybasedRoutingTest {
	...
	public void the2NodesShouldFindEachOther() throws Throwable {
		...
		kbrs.get(1).joinURI(
				Arrays.asList(new URI("openkad.udp://127.0.0.1:" + basePort
						+ "/")));
	...
	public void the16NodesShouldFindEachOther() throws Throwable {
		...
		for (int i = 1; i < kbrs.size(); ++i) {
			...
			kbrs.get(i).joinURI(
					Arrays.asList(new URI("openkad.udp://127.0.0.1:" + port
							+ "/")));
		}
		...
		for (int j = 0; j < kbrs.size(); ++j) {
			...
			for (int i = 0; i < kbrs.size(); ++i) {
				List<Node> findNode = kbrs.get(i).findNode(
						kbrs.get(j).getLocalNode().getKey());
				System.out.println("i=" + i + ", j=" + j + ", findNode="
						+ findNode);
	 ...
	 * System.out.println(i+" ==> "+(i-1));
	 * kbrs.get(i).joinURI(Arrays.asList(new
	...
	public void the2NodesShouldAbleToSendMessages() throws Throwable {
		...
		List<Node> findNode = kbrs.get(0).findNode(
				kbrs.get(1).getLocalNode().getKey());
	...
	public void the16NodesShouldAbleToSendMessages() throws Throwable {
		...
		List<Node> findNode = kbrs.get(0).findNode(
				kbrs.get(13).getLocalNode().getKey());
	...
	private static final class X implements Serializable {
		...
		public boolean equals(Object obj) {
			if (obj == null || !getClass().equals(obj.getClass()))
				return false;
			X x = (X) obj;
			return this == obj || (x.a == a && x.b == b && x.c.equals(c));
		}
	}

	@Test(timeout = 5000)
	public void the2NodesShouldAbleToSendArbitrarySerializableMessages()
			throws Throwable {
		int basePort = 10400;
		List<KeybasedRouting> kbrs = new ArrayList<KeybasedRouting>();
		for (int i = 0; i < 2; ++i) {
	...
	public void the2NodesShouldAbleToSendRequest() throws Throwable {
		...
		Serializable res = kbrs.get(0)
				.sendRequest(findNode.get(0), "tag", "msg").get();
		Assert.assertEquals("new_msg", res);
	}

	@Test(timeout = 5000)
	public void the2NodesShouldAbleToSendArbitrarySerializableRequests()
			throws Throwable {
		int basePort = 10500;
		List<KeybasedRouting> kbrs = new ArrayList<KeybasedRouting>();
		for (int i = 0; i < 2; ++i) {
		...
		Serializable res = kbrs.get(0).sendRequest(findNode.get(0), "tag", x)
				.get();
